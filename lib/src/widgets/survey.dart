...
import 'package:flutter/material.dart';
import 'package:flutter_survey/src/extensions/deep_copy.dart';
import 'package:flutter_survey/src/util/custom_diff_util_sliver_list.dart';
import '../models/question.dart';
import '../models/question_result.dart';
import 'question_card.dart';
...
class _SurveyState extends State<Survey> {
  late List<Question> _surveyState;
  late Widget Function(BuildContext context, Question question, void Function(List<String>) update)
      builder;

  @override
  void initState() {
    // Clone the survey tree so that changes don’t affect the original data.
    _surveyState = widget.initialData.map((question) => question.clone()).toList();
  ...
  @override
  Widget build(BuildContext context) {
    // Build the list of widgets recursively using your original _buildChildren function.
    var children = _buildChildren(_surveyState);

    return CustomScrollView(
      slivers: [
        // Use our custom diff util sliver list to animate changes.
        CustomDiffUtilSliverList.fromKeyedWidgetList(
          children: children,
          insertAnimationBuilder: (context, animation, child) => FadeTransition(
            opacity: animation,
            child: child,
